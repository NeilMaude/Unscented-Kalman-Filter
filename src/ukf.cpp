...
void UKF::ProcessMeasurement(MeasurementPackage meas_package) {
  ...
  */
  ...
  if (is_initialized_ != true) {
    ...
    //std::cout << "Weights size = " << weights_.size() << std::endl;
...
void UKF::Prediction(double delta_t) {
  ...
  */
  ...
  std::cout << "Created Sigma Points = " << std::endl << Xsig << std::endl;     // test code
  */
  ...
  std::cout << "Created Augmented Sigma Points = " << std::endl << Xsig_aug << std::endl;     // test code

  // Predict these Sigma points
  MatrixXd Xsig_pred = MatrixXd(n_x_, 2 * n_aug_ + 1);
  SigmaPointPrediction(Xsig_aug, delta_t, &Xsig_pred);

  std::cout << "Predicted Sigma Points = " << std::endl << Xsig_pred << std::endl;     // test code
...
void UKF::UpdateRadar(MeasurementPackage meas_package) {
  ...
  */
  ...
  //add measurement noise covariance matrix       - ** can calc R just once in initialisation?? **
  MatrixXd R = MatrixXd(n_z, n_z);
